...
import {logger, types} from '@opencensus/opencensus-core';
...
import {Batch, Process} from './jaeger-thrift';
import {SenderCallback} from './reporter';
...
export default class UDPSender {
  ...
  constructor(options: any = {}) {
    ...
    this._logger = options.logger || logger.logger();
  ...
  _calcBatchSize(batch: Batch) {
    return this._agentThrift.Agent.emitBatch.argumentsMessageRW
        .byteLength(this._convertBatchToThriftMessage())
        .length;
  }

  _calcSpanSize(span: any): LengthResult {
    return this._jaegerThrift.Span.rw.byteLength(
        new this._jaegerThrift.Span(span));
  }

  setProcess(process: Process): void {
    // This function is only called once during reporter construction, and thus
    // will give us the length of the batch before any spans have been added to
    // the span list in batch.
  ...
  _invokeCallback(
      callback?: SenderCallback, numSpans?: number, error?: string) {
    if (callback) {
  ...
  append(span: any, callback?: SenderCallback): void {
    ...
    if (err) {
      this._invokeCallback(
          callback, 1, `error converting span to Thrift: ${err}`);
      return;
    }
    ...
    if (spanSize > this._maxSpanBytes) {
      this._invokeCallback(
          callback, 1,
          `span size ${spanSize} is larger than maxSpanSize ${
              this._maxSpanBytes}`);
      return;
    }
    ...
    this.flush((numSpans: number, err?: string) => {
      // TODO theoretically we can have buffer overflow here too, if many spans
      // were appended during flush()
      this._batch.spans.push(span);
      this._totalSpanBytes += spanSize;
  ...
  flush(callback?: SenderCallback): void {
    ...
    const writeResult =
        this._agentThrift.Agent.emitBatch.argumentsMessageRW.writeInto(
            this._convertBatchToThriftMessage(), thriftBuffer, 0);
    this._reset();
    ...
    // Having the error callback here does not prevent uncaught exception from
    // being thrown, that's why in the constructor we also add a general
    // on('error') handler.
    this._client.send(
        thriftBuffer, 0, thriftBuffer.length, this._port, this._host,
        (err, sent) => {
          if (err) {
            const error: string = err &&
                `error sending spans over UDP: ${err}, packet size: ${
                                      writeResult.offset}, bytes sent: ${sent}`;
